...
def nn(modfile, cls):
    ...
    if not mine: return f"{modfile:26s} -> {cls}: AUCUNE position extraite"
    dists=[]
    for m in mod:
    ...
    return f"{modfile:26s} ({len(mod):4d} pts) vs {cls} ({len(mine):4d}): médiane={med:8.1f}u  <100u: {within100:5.1f}%  max={dists[-1]:9.1f}"
